...
fn build_system_prompt(agglutinative: bool) -> String {
    ...
    prompt.push_str(
        "1. Every variant of the Morphology enum MUST have `lemma: String` as the FIRST field.\n\
         ...
         9. Use UPOS tag set for morphology variants (Adjective, Adposition, Adverb, CoordinatingConjunction, Determiner, Interjection, Noun, Numeral, Particle, Pronoun, ProperNoun, SubordinatingConjunction, Symbol, Verb, Other). Tag auxiliaries as Verb (no separate Auxiliary category), and do NOT declare a Punctuation variant — punctuation is not analysed.\n"
    );
