...
impl LinguisticDefinition for EgyptianArabic {
    ...
    fn extraction_directives(&self) -> &'static str {
        "1. Scope and lemmatization: analyze contemporary Egyptian Arabic (`arz`), with neutral Cairene usage as the default, and NEVER silently normalize it into Modern Standard Arabic. Lemmatize verbs to the third-person masculine singular perfective (بيكتب/حيكتب/اكتب -> كتب; بيتكلم -> اتكلم), nouns to the indefinite singular, adjectives to the masculine singular lexical or participial form, and pronouns/determiners to their independent citation form. There is no infinitive citation form.\n\
         ...
         14. Names, address and formulas: a personal or place name is `proper_noun`, which takes only a lemma — `سارة`، `منى`، `علي`، `أحمد`، `مصر`، `إسكندرية`. NEVER tag a name as `noun`, and never manufacture a root for one: `علي` the name has no root, unlike the preposition `على`. The vocative `يا` is a `particle` whose `particle_function` is `vocative`, never a bare particle without that field. Greeting and reaction formulas — `أهلاً`، `شكراً`، `معلش`، `يلا`، `خلاص`، `إن شاء الله` — are `interjection`, not nouns, and take only a lemma. Every `particle` you emit MUST carry a `particle_function`, and every `noun` MUST carry gender, number, definiteness and state.\n\
