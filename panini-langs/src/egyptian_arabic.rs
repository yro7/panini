...
impl LinguisticDefinition for EgyptianArabic {
    ...
    fn extraction_directives(&self) -> &'static str {
        "1. Scope and lemmatization: analyze contemporary Egyptian Arabic (`arz`), with neutral Cairene usage as the default, and NEVER silently normalize it into Modern Standard Arabic. Lemmatize verbs to the third-person masculine singular perfective (بيكتب/حيكتب/اكتب -> كتب; بيتكلم -> اتكلم), nouns to the indefinite singular, adjectives to the masculine singular lexical or participial form, and pronouns/determiners to their independent citation form. A lemma is written without any diacritic, tanwin included: `أهلاً` -> `أهلا`, `شكراً` -> `شكرا`, `إنتِ` -> `إنت`. There is no infinitive citation form.\n\
         2. Canonical script: keep `word` exactly in the source script, every diacritic and tanwin included (`شكراً` stays `شكراً`, `إنتِ` stays `إنتِ`). Write every lemma and root in CODA-style Arabic script so Arabic and Latin/Arabizi input aggregate to one lexicon. Write a root as hyphen-separated radicals (`ك-ت-ب`, `ك-ل-م`, including four radicals when real); a hamza radical is written `أ`, never bare `ا` (`متأخر` has `أ-خ-ر`). Preserve Egyptian spellings and NEVER replace an Egyptian lemma with an MSA cognate.\n\
         3. Roots: include `root` only when a synchronically transparent Arabic root exists. `كتاب` has `ك-ت-ب`; by contrast `مكالمة` has `ك-ل-م`. OMIT root for an opaque loan such as `ترابيزة` and for a loan adjective such as `شيك`; never manufacture a root from every consonant in a loan. OMIT root for `كويس` (fine), `اسم` (name) and `حضرة` (the polite address in `حضرتك`): neither is synchronically transparent, and `اسم` must never receive `س-م-ي` or `س-م-و`. Every ordinary patterned verb must have a root, including a real quadriliteral such as `ترجم` (`ت-ر-ج-م`).\n\
         4. Nouns: always report lexical gender, singular/dual/plural number, definiteness and state. Egyptian has a productive nominal dual (`كتابين`) but NO nominal case endings. `كتاب الطالب` is construct and definite, while `كتاب طالب` is construct and indefinite; a possessive suffix also puts its host in construct state and makes it definite (`كتابه`). A feminine noun writes its `ة` as `ت` before a suffix, but its lemma keeps the `ة`: `حضرتك` is `حضرت` plus `ك` with lemma `حضرة`, `مدرستي` is `مدرست` plus `ي` with lemma `مدرسة`. Free nouns are `state: free`.\n\
         ...
         8. Determiners: classify the attached article `الـ`, attributive demonstratives, quantifiers and interrogatives. Every demonstrative has the lemma `ده` (`ده`, `دي`, `دول`, `دا` -> `ده`). Demonstratives encode referent_number (`ده` singular versus `دول` plural), while the article `الـ` encodes none and MUST omit it. Singular demonstratives encode referent_gender (`ده` masculine versus `دي` feminine); OMIT it on gender-neutral plural `دول`, on `الـ`, and on any determiner whose form carries no gender contrast.\n\
         9. Pronouns: `clitic` is true only for an attached pronoun. For `كتابه`, attachment_function is `possessive`; for `شفته`, it is `direct_object`; contrast `كتب له`/an attached indirect-object sequence with a preposition-hosted suffix such as `معاه`. An attached pronoun's lemma is the independent pronoun of the same person and number, never the clitic itself: ني/ي/يا -> `أنا`, ك (masculine -ak and feminine -ik alike) -> `إنت`, ه/ـه/و -> `هو`, ها -> `هي`, نا -> `إحنا`, كم/كو -> `إنتو`, هم -> `هم`; the -ak/-ik contrast is carried by referent_gender, not by the lemma. The second-person singular clitic `ك` ALWAYS contrasts masculine -ak with feminine -ik even though both are written `ك`: set its referent_gender whenever the card shows the addressee's gender (a name such as `أحمد`/`منى`, a title such as `أستاذة`/`مدام`, a gendered verb or adjective, or the translation), on `إزيك`, `حضرتك`, `اسمك` and `أشوفك` alike, and omit it only when nothing in the card shows it. OMIT attachment_function on independent `هو`. Set referent_person for personal forms (`أنا` first versus `إنت` second) and OMIT it for `اللي`; set referent_number for `أنا` singular versus `إحنا` plural and OMIT it for `مين`; set referent_gender for `هو` masculine versus `هي` feminine and OMIT it for `أنا`, `إحنا`, `هم`, `اللي` and forms without a gender contrast. Egyptian has no dual pronoun.\n\
         ...
         11. Tokenization: ALWAYS split these productive clitics into their own analyses, in every sentence and never only sometimes — conjunctions `و-/ف-`, prepositions `ب-/ل-`, the article `ال-`, negative `ما-/-ش`, and attached pronouns — while retaining their syntactic effect on the host, so `البيت` is `ال` plus `بيت` and `ماكانتش` is `ما` plus `كانت` plus `ش`. Do NOT split the aspect prefix `ب-` or the future prefix `ح-/هـ-`: those are recorded on the verb through `verb_form` as `bi_imperfective` and `ha_imperfective`, and must never also surface as a separate particle. Do NOT split person/number/gender inflection (`يـ/تـ/أ-/نـ`, `-ت`, `-ي`, `-وا`). When you split a written token, each analysis's `word` is exactly the piece it analyses; the pieces must not overlap and must never restate the whole token, so `عليها` is `علي` plus `ها` and never `عليها` alongside `ها`. This applies to EVERY host, including ones that look like single words: `عليكم` is `علي` plus `كم`, `إزيك` is `إزي` plus `ك`, `إزيها` is `إزي` plus `ها`, `معاه` is `معا` plus `ه`, `كتابه` is `كتاب` plus `ه`, `أشوفك` is `أشوف` plus `ك`, and `لله` in `الحمد لله` is the preposition `ل` plus `له` (a `proper_noun` with lemma `الله`). A host must always reach the lexicon under its bare surface, so never record `عليكم` or `إزيك` as a whole surface for the lemma `على` or `إزاي`. Omit punctuation.\n\
         ...
         13. Conjunctions: decide coordination versus subordination for every conjunction and emit the specific value. `coordinating_conjunction` covers و، ف، أو، بس، لكن; `subordinating_conjunction` covers لما، عشان، علشان، إن، إنه، لو، لأن، قبل ما، بعد ما، من غير ما، بدل ما. NEVER emit a bare `conjunction` or `conj`: an unspecific tag is filed as coordinating and silently mis-analyses Egyptian subordination. `اللي` is not a conjunction; it is a relative pronoun.\n\
