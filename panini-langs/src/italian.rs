...
pub enum ItalianTense {
    ...
    /// participio presente).
    Present,
    /// Participio passato: the tense of a participle, never of a finite form.
    /// Compound tenses are split, so this is the second half of every one of
    /// them — `ho parlato`, `avevo parlato`, `avrei parlato`.
    Past,
    /// Passato remoto — the synthetic simple past, one token: `parlai`, `fu`.
    SimplePast,
    /// Futuro semplice.
    Future,
    /// Imperfetto, indicative and subjunctive alike.
    Imperfect,
}
...
impl LinguisticDefinition for Italian {
    ...
    fn extraction_directives(&self) -> &'static str {
        "1. Lemmatization: provide the dictionary form — infinitive for verbs, masculine singular for nouns, adjectives and determiners.\n\
         ...
         4. Tense values: presente -> present, imperfetto -> imperfect, futuro semplice -> future, passato remoto -> simple_past. Reserve past for the participio passato — it is the tense of a participle and never of a finite form, so a finite verb must never be tagged past.\n\
         5. Verbs: always specify Tense and Mood. Specify Person and Number only for finite forms; omit both for infinitive, gerund and participle.\n\
         6. Past participles: add Gender and Number only when the form actually marks agreement — with the subject under 'essere' ('e andata', 'sono partiti') and with a preceding direct-object clitic under 'avere' ('l'ho vista', 'le ho comprate'). Omit them for the invariable form ('ho comprato le mele').\n\
         ...
         8. Clitic clusters: split an attached or combined clitic group into the verb and each clitic, each keeping its own lemma — dammelo -> 'dare' + 'me' + 'lo'; andarci -> 'andare' + 'ci'; parlandogli -> 'parlare' + 'gli'; glielo -> 'gli' + 'lo'.\n\
