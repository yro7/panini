...
## 📊 Curated Lexicon Pivots

Language macros generate typed `PivotField` handles from real morphology and grammatical-function fields. Use these handles when exposing analysis pivots to an API or UI instead of writing string keys by hand.
...
Closed pivots expose their known value inventory:

```rust
let pivot = PolishMorphology::PIVOT_CASE;
assert_eq!(pivot.key, "case");
let allowed_values = pivot.values();
```

Open pivots, such as Arabic `root: String`, still have a stable key and typed extractor but no finite value list. Agglutinative languages can additionally curate morpheme pivots with handles generated by `#[derive(GrammaticalFunctionCatalog)]`, for example `TurkishGrammaticalFunction::PIVOT_POLARITY`.
