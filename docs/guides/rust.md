...
Language macros generate typed `PivotField` handles from real morphology and morpheme-function fields. Use these handles when exposing analysis pivots to an API or UI instead of writing string keys by hand.
...
Open pivots, such as Arabic `root: String`, still have a stable key and typed extractor but no finite value list. Agglutinative languages can additionally curate morpheme pivots with handles generated by `#[derive(MorphemeFunctionCatalog)]`, for example `TurkishMorphemeFunction::PIVOT_POLARITY`.
