...
4. **Curate Analysis Pivots**: Choose which typed morphology or morpheme fields are useful enough to expose to downstream lexicon dashboards.
...
- `MORPHOLOGY_PIVOTS` for fields on the language morphology enum, such as Polish `case` or Arabic `root`.
- `MORPHEME_PIVOTS` for fields on grammatical-function enums, such as Turkish morpheme `polarity`, `tense`, or `case`.
